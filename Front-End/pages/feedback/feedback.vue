<template>
	<view class="feedback-root">
		...
		<view class="feedback-type-wrapper common-padding feedback-item-margin-top">
			...
			<radio-group class="feedback-type" @change="radioChange">
				<label class="feedback-type-radio">
					<radio value="QUESTION"  checked /><text>建议与反馈</text>
				</label>
				<label class="feedback-type-radio">
					<radio value="SUGGESTION"  /><text>举报恶意用户</text>
				</label>
				<label class="feedback-type-radio">
...
<!-- 内容 -->
		<view v-if="type=='SUGGESTION'" class="feedback-content-wrapper common-padding feedback-item-margin-top">
			...
			<textarea class="feedback-content" v-model="content" required placeholder-class="feedback-content-placeholder" value=""
			 placeholder="请输入举报原因(如:言语辱骂)，并上传相关截图证据，我们将严肃处理。" />
		...
		<view  class="feedback-submit" @click="confirmPopup">
			提交
		</view>
		<!-- 弹窗 -->
		<uni-popup v-if="type!=='SUGGESTION'" ref="popup" type="center">
		...
		<uni-popup v-if="type=='SUGGESTION'" ref="popup" type="center">
			<view class="popup-content">
				...
				<view class="popup-button">
					...
					<view class="popup-confirm"  @click="confirmPopup2">
						确认
					</view>
				</view>
			</view>
