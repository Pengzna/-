<template>
	<view>
		...
		<view class="textarea-wrapper">
			<textarea v-model="textContent" maxlength=200 class="textarea" :placeholder="publishType === 'REPOST' ? '说一说我的看法...' : '今天有什么想分享的鸭~' "
...
<form>
					<view class="cu-form-group margin-top">
						<view class="title" font-size="24rpx">选择所属类别:</view>
						<picker @change="PickerChange"  :value="index" :range="picker" >
							
							<view class="picker">
								{{index>-1?picker[index]:'默认为其他类别'}}
							</view>
						</picker>
		...
		<view class="edit-info">
			<view class="edit-info-image-wrapper">
				<view  class="edit-info-image-view">
					<image v-if="publishType!='REPOST'" src="../../static/icon/publish_photo.png" class="edit-info-image" @click="chooseImage"></image>
				</view>
				<!-- <view class="edit-info-image-view">
...
<script>
	...
	export default {
		...
		computed: {
			momentType() {this.imageTempList.length
				//if (this.imageList && this.imageList.length > 0) {
				if (this.imageTempList && this.imageTempList.length > 0) {
					return "IMAGE";
		...
		methods: {
			...
			getImageList(){
				...
				// 								let imageTemp = this.imageTempList[i];
				// 						uploadImage(imageTemp.path, 'images/',url => {
				// 										console.log(url);
				// 										this.imageList.push({
				// 											url,
				// 											width: imageTemp.width,
