			...
			chooseImage() {
				uni.chooseImage({
					...
					success: async (res) => {
						...
						if (this.imageTempList.length + res.tempFilePaths.length > 3) {
							uni.showToast({
								title: "最多上传3张图片",
								duration: 2000
							})
			...
			async publish() {
				if(this.textContent.length==0){
					...
					if (this.publishType === 'REPOST') {
						...
						param.activityId=this.moment.activityId;
					...
					// 	}
						...
						param.activityId=this.moment.activityId;
						console.log(param);
						let [publishMomentData, publishMomentError] = await httpUtils.postJson("/moment/editMoment", param);
						uni.hideLoading();
