<template>
 ...
 -->				<input ref="inputValue" :focus="inputFocus" placeholder="可以不填,不要恶意乱写哦"  class="popup-desc" type="text" v-model="currentFieldValue" v-if="currentFieldName === 'DESCRIPTION' " />
				<input ref="inputValue" :focus="inputFocus" placeholder="想一个好听的昵称"  class="popup-desc" type="text" v-model="currentFieldValue" v-if="currentFieldName === 'NAME' " />
		...
		methods: {
			...
			async confirmPopup() {
				if (this.currentFieldName === "NAME") {
					if (this.currentFieldValue && this.currentFieldValue.length > 10) {
						uni.showToast({
							title: "昵称不能多于10字符",
							duration: 2000
						});
