<template>
	<view>
		...
		<view class="textarea-wrapper">
			<textarea v-model="textContent" maxlength=200 class="textarea" 
...
imgUrl: 'https://wpimg.wallstcn.com/69a1c46c-eb1c-4b46-8bd4-e9e686ef5251.png',
				...
				videoCoverList: [],
				momentId:"",
				momentIdList:"",//用来存放从上一个页面获取的list
				// 临时图片列表
				imageTempList: [
									],
				// 图片列表
		...
		async onLoad(options) {
			...
			//console.log(options.momentIdList[0]);
			this.momentIdList=options.momentIdList;
			let items=this.momentIdList;
			...
			for (var i = 0, lenI = this.arr.length; i < lenI; i++) {
				...
				uni.hideLoading();
				if(this.moment.imageList){
					this.textContent=this.textContent+"第"+(i+1)+"条："+this.moment.textContent+"（带图）\n";
				}else{
					this.textContent=this.textContent+"第"+(i+1)+"条："+this.moment.textContent+"\n";
				}
				
				for (let i in this.moment.imageList) {
					//let tempFilePath = this.moment.imageList[i].url;
					let tempFilePath = this.moment.imageList[i].url.substring(0,this.moment.imageList[i].url.lastIndexOf("\?"));
		...
		getImageList(){
				for (let i in this.imageTempList) {
					let imageTemp = this.imageTempList[i];
							if(imageTemp.path.substring(8,12)=="live"){
								console.log(imageTemp.path)
								this.imageList.push({
									url:imageTemp.path,
		...
		// 			}
					...
					if (!publishMomentError) {
						...
						for(let i=0;i<this.arr.length;i++){
							...
							let [publishMomentDataa, publishMomentErrora] = await httpUtils.postJson("/moment/deleteMoment", param2);
