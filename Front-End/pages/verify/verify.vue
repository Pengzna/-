<template>
	<view >
		...
		<!-- <checkbox-group  @change="checkboxChange">
			<view v-for="(moment) in momentInfo.momentList" >
				
				<checkbox :value="moment.id" :checked="false" />
				
					<verifymoment   :moment="moment" momentPageType="LIST">
		...
		<block >
			...
			<view class='buy-btn '>
						...
						<view class='right'>
							<view>已选{{values.length}}个
								
							</view>
							<view @click='toPublishMoment(values)'>去发布</view>
						</view>
					</view>
			</block>
		...
		<!-- <swiper class="tab-content" :duration="300" :current="curTabIndex" @change="swiperChange" acceleration="false">
			<swiper-item class="tab-content-item">
				<verifymoment v-for="(moment) in momentInfo.momentList" :moment="moment" momentPageType="LIST" v-on:updateMomentLikeCount="updateMomentLikeCount($event,moment)"
...
<script>
	...
	import verifymoment from '../../components/verifymoment/verifymoment.vue';
	...
	export default {
		...
		methods: {
			...
			async loadMomentInfo() {
				let [data] = await httpUtils.postJson("/square/queryVerifyMoment", {
					pageNo: this.momentInfo.pageNo,
					pageSize: this.momentInfo.pageSize,
					status: this.getLoginUser().userId
				});
			...
			checkboxChange: function (e) {
				...
				var items = this.momentInfo.momentList;
					this.values = e.detail.value;
					console.log(this.values);
				for (var i = 0, lenI = items.length; i < lenI; i++) {
					const item = items[i]
					//console.log(item);
					if(this.values.indexOf(item.value) >=0){
...
toPublishMoment(momentIdList) {
				uni.navigateTo({
					url: `../verifyPublish/verifyPublish?publishType=MOMENT&momentIdList=${momentIdList}`
				});
			},
			// 加载image数据
			async loadImageInfo() {
