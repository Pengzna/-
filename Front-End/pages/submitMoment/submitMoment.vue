<template>
	<view>
		...
		<view class="textarea-wrapper">
			<textarea v-model="textContent" maxlength=200 class="textarea" :placeholder="publishType === 'REPOST' ? '说说我的看法...' : '只能投递给已关注的墙墙组织~   请用简短文字描述，附带一张图片提交哦~' "
		...
		<view class="edit-info">
			...
			<form>
				<view class="cu-form-group margin-top">
					<view class="title" font-size="24rpx">投给已关注的墙:</view>
					<picker @change="PickerChange"  :value="index" :range="uniinfo" :range-key="'name'">
						
						<view class="picker">
							{{uniinfo.length>0?uniinfo[index].name:'暂无已关注墙'}}
						</view>
					</picker>
...
imgUrl: 'https://wpimg.wallstcn.com/69a1c46c-eb1c-4b46-8bd4-e9e686ef5251.png',
		...
		async onLoad(options) {
			if(options.momentId){
				...
				uni.hideLoading();
				this.textContent=this.moment.textContent;
				for (let i in this.moment.imageList) {
					let tempFilePath = this.moment.imageList[i].url;
			...
			// console.log(this.imageTempList.find());
			console.log(this.uniinfo[1]);
			// this.imageTempList[0].path=this.imgUrl;
			// this.imageTempList[0].width=500;
			// this.imageTempList[0].height=500;
			// this.imageTempList[1].path=this.imgUrl;
			// this.imageTempList[1].width=500;
			// this.imageTempList[1].height=500;
			for (let i in this.uniinfo) {
				this.picker[i]=this.uniinfo[i].name;
				console.log(this.uniinfo[i].name);
			}
		...
		methods: {
			async loadFollowInfo() {
				...
				for(let i=0;i<this.followInfo.userList.length;i++){
					
					this.uniinfo.push({
						id: this.followInfo.userList[i].id,
						name: this.followInfo.userList[i].name,
						
					});
				}
				console.log(this.uniinfo);
			...
			async publish() {
				if(this.textContent.length==0){
					...
					if (this.publishType === 'REPOST') {
						...
						console.log(this.uniinfo[this.index])
						if((this.uniinfo[this.index])){
							...
							await this.getImageList();
								...
								if (this.momentType === 'VIDEO') {
									...
									param.status = this.uniinfo[this.index].id;//提交给谁，status就写谁的id
									console.log(this.uniinfo[this.index].id);
								let [publishMomentData, publishMomentError] = await httpUtils.postJson("/moment/submitMoment", param);
								uni.hideLoading();
